...
export interface PageChangerProps<TData, TValue> {
    searchParams: { [key: string]: number | string | string[] | undefined, page: number, pageSize: number };
...
export function PageChanger<TData, TValue>({
    ...
    function setPage(page: number) {
        ...
        router.replace(`${prefixUrl}?page=${page}&pageSize=${searchParams.pageSize * 1}`);
    }
    function setPageSize(page: number) {

        router.replace(`${prefixUrl}?page=${searchParams.page}&pageSize=${page * 1}`);
    }
